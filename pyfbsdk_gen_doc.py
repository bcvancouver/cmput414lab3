...
def FBMessageBoxWithCheck(pBoxTitle, pMessage, pButton1Str, pButton2Str, pButton3Str, pCheckBoxStr, pCheckBoxValue,
                          pDefaultButton, pScrolledMessage):
    """
...
def FBMessageBoxGetUserValue(pBoxTitle, pMessage, pValue, pValueType, pButton1Str, pButton2Str, pButton3Str,
                             pDefaultButton):
    """
...
class FBPythonWrapper(object):
    ...
    """
    OnUnbind = property(
        doc="<b>Event:</b> Will notifier the user when the corresponding ORSDK objects is unbound from the PythonObject.         ")
    pass
...
class FBTimeCode(object):
    ...
    MPAL_30 = property(
        doc="-29.971f Currently not supported : '1' is added just to differentiate from NTSC_FULL(-29.97f)         ")
...
class FBCallback(object):
    ...
    Callback = property(
        doc="<b>Read Property:</b> Python callback that will called when the FBCallback is executed.         ")
    pass
...
class FBCharacterPoseOptions(object):
    ...
    mMirrorPlaneEquation = property(
        doc="Mirror plane equation (used when mMirrorPlaneType = kFBMirrorPlaneTypeEquation).         ")
    mMirrorPlanePanAngle = property(
        doc="Mirror plane pan angle in degrees (used when mMirrorPlaneType = kFBMirrorPlaneTypeUser).         ")
    mMirrorPlaneTiltAngle = property(
        doc="Mirror plane tilt angle in degrees (used when mMirrorPlaneType = kFBMirrorPlaneTypeUser).         ")
...
class FBVideoCodecManager(object):
    ...
    VideoCodecMode = property(
        doc="<b>Read Write Property:</b> This decide how the system behaves when ask to render a file (codec dialog, uncompress, use default codec)         ")
    pass
...
class FBVideoGrabOptions(object):
    ...
    RenderAudio = property(
        doc="<b>Read Write Property:</b> If true and there's audio in the scene, add audio to the output file.         ")
    AudioRenderFormat = property(doc="<b>PRead Write roperty:</b> Audio render format.         ")
    StillImageCompression = property(
        doc="<b>Property:</b> Compression ratio for image(jpg) 0-100 where 0=Greatest compression, 100=Least Compression.         ")
    pass
...
class FBAddRegionParam(object):
    ...
    mPos = property(
        doc="<b>Read Property:</b> Offset in pixel according depending on the use of FBAddRegionParam (X, Y, W or H).         ")
...
class FBPlotOptions(object):
    ...
    UseConstantKeyReducer = property(
        doc="<b>Read Write Property:</b> Should we use a constant key reducer with the filter?         ")
    ConstantKeyReducerKeepOneKey = property(
        doc="<b>Read Write Property:</b> Should the constant key reducer keep at least one key?         ")
    PlotTranslationOnRootOnly = property(
        doc="<b>Read Write Property:</b> Should we plot the translation on root only?         ")
...
class FBAudioRenderOptions(object):
    ...
    BitDepthMode = property(
        doc="<b>Property:</b> Bit depth for one sample of audio. 8 bits and 16 bits available for audio render, 16 bits default.         ")
    ChannelMode = property(
        doc="<b>Property:</b> Audio render channel number, 1 for Mono(left channel right channel render mixed to one channel), 2 for Stereo(left channel right channel render separately).         ")
    OutputFileName = property(doc="<b>Property:</b> Audio Render destination file.         ")
    RateMode = property(
        doc="<b>Property:</b> Rate mode for number of samples per second. 44100 hz default,8000, 11025,12000,16000,22050,24000,32000,44100,48000,64000,88200,96000 available for audio render.         ")
...
class FBBatchOptions(object):
    ...
    FrameAnimation = property(
        doc="<b>Read Write Property:</b> Set timeline start and end time to corespond with the start and end of animation.         ")
    ...
    WriteTranslation = property(
        doc="<b>Read Write Property:</b> Write translation animation data included with Acclaim AMC files.         ")
    ...
    KeepCharacterConstraint = property(
        doc="<b>Read Write Property:</b> To keep the character constaint when saving.         ")
    OnTakeExistAction = property(
        doc="<b>Read Write Property:</b> Action to perform when a take already exist while in a batch process.         ")
    OnContainsBatchTakesAction = property(
        doc="<b>Read Write Property:</b> Action to perform when a scene already contains batch takes while in a batch process.         ")
    pass
...
class FBTangentConstantMode(Enumeration):
    ...
    """
    kFBTangentConstantModeNormal = property(
        doc="The tangent will contain the value of the current keyframe until the next keyframe.         ")
...
class FBProfilingMode(Enumeration):
    ...
    """
    kFBProfilingModeDisabled = property(
        doc="All profiling disabled, this include Viewer profiling. For the other modes, when EvaluationDepth is 0, only base information is profiled, such as FPS and evaluation rate.         ")
    kFBProfilingModeEvaluation = property(
        doc="Collect profiling for all known evaluation tasks (default mode).         ")
    ...
    kFBProfilingModeAllLow = property(
        doc="Collect profiling for all known tasks that doesn't increase remarkably with scene size. For large scenes this will not influence performance.         ")
    kFBProfilingModeAllHi = property(
        doc="Collect profiling for all known tasks . For large scenes there should be an influence on performance.         ")
    pass
...
class FBProperty(FBPlug):
    ...
    Data = property(
        doc="<b>Read Write Property:</b> The property data value. Type of this depends on the subclass of FBProperty (ex: in a FBPropertyInt, Data is of type int).         ")
    pass
...
class FBCellStyle(Enumeration):
    ...
    kFBCellStyleView = property(
        doc="View (user definable, you need to specify the view using FBSpread::SetCellView()).         ")
...
class FBPropertyFlag(Enumeration):
    ...
    kFBPropertyFlagHideProperty = property(
        doc="This flag is used to show/hide the property in the propertiview. However, when turn on/off HidePropertry flag, this property won't show/hide unless you reload the UI. The nodes hidden by this flag are removed from UI.         ")
    ...
    kFBDynamicHidden = property(
        doc="This flag is used to show/hide the property in the propertiview. When turn on/ff DynamicHidden flag, this property will show/hide. The nodes hidden by this flag still exist in UI.         ")
    kFBSlaveSetByMaster = property(
        doc="Slave property can be modified, valid only when the master property is modified.         ")
...
class FBComponent(FBPlug):
    ...
    Name = property(
        doc="<b>Read Write Property:</b> Unique name of object. See sample: RemoveSuffixFromNameOfSceneElements.py.         ")
...
class FBLayerMode(Enumeration):
    ...
    kFBLayerModeAdditive = property(
        doc="Layer value will be added to the other layers to computed the final value.         ")
    kFBLayerModeOverride = property(doc="Layer value will override the value of the other precedent layers.         ")
    kFBLayerModeOverridePassthrough = property(
        doc="If the layer has a weigth of 75%, the precedent layers will have a combined effect of 25% on the final value. Setting the weigth to 100% is similar to setting the layer in override.         ")
    pass
...
class FBMergeLayerMode(Enumeration):
    ...
    """
    kFBMergeLayerModeAutomatic = property(
        doc="The resulting layer will be in override mode if one of the source layer is in override, otherwise, it will be in additive mode.         ")
...
class FBCharacterLoadAnimationMethod(Enumeration):
    ...
    kFBCharacterLoadRetarget = property(
        doc="Retarget (copy and correct) keys from loaded character to target character.         ")
    kFBCharacterLoadPlotIfSampled = property(
        doc="If loaded animation seems sampled, plot animation from loaded character to target character; else retarget.         ")
...
class FBPlugModificationFlag(Enumeration):
    ...
    kFBSelfDataModified = property(
        doc="Object/Property itself has been dirty, in case of property get dirty, its owner object will be set dirty as well.         ")
    ...
    kFBSelfKeyingModified = property(
        doc="Object/Property itself has been dirty, in case of property get dirty, its owner object will be set dirty as well.         ")
    ...
    kFBContentConnectionModified = property(
        doc="Owner object/namespace has connection modified property/objects.         ")
...
class FBObjectFlag(Enumeration):
    ...
    kFBFlagParentable = property(
        doc="Object (model) can be 'parented'. Used by the apply manager contextual menu.         ")
    ...
    kFBFlagUndoableSeparately = property(
        doc="Object which has kFlagUndoableSeparately flag turned on will have a separate Undo Stack.         ")
    ...
    kFBFlagStory = property(
        doc="Object created/used by the Story tool. Useful flag for filtering Story objects.         ")
...
class FBTangentMode(Enumeration):
    ...
    """
    kFBTangentModeAuto = property(
        doc="This is the equivalent to a cardinal spline with no parametrization. In the UI, it is identified as Smooth.         ")
    kFBTangentModeTCB = property(doc="TCB spline (3 parameters: TENSION, CONTINUITY, BIAS)         ")
    kFBTangentModeUser = property(
        doc="Used to represent all splines with no lost data (HERMITE, BEZIER, CATMUL, etc.)         ")
    kFBTangentModeBreak = property(doc="Like USER but left slope may differ from right.         ")
    kFBTangentModeTimeIndependent = property(
        doc="Time independent, is calculated based upon the slope between the previous and next key values. In the UI, it is identified as Spline.         ")
    kFBTangentModeClampProgressive = property(
        doc="Time independent, will flatten the tangent handles when the key value goes over or under the previous and next key values. In the UI, it is identified as Auto.         ")
    pass
...
class FBStereoDisplayMode(Enumeration):
    ...
    kFBStereoDisplayActive = property(
        doc="Display in active mode. User must enable OpenGL quad stereo buffer, and choose approriate stereo mode in video card hardware's config app.         ")
...
class FBShadowType(Enumeration):
    ...
    """
    kFBShadowTypeShadowPlanar = property(
        doc="Use this shadow type to create darkened shadow areas only on planar surfaces.         ")
    ...
    kFBShadowTypeZShadowProjectiveTexture = property(
        doc="Similar to the Projective Shadow, except that it uses a boolean algorithm to create a self-shadow.         ")
    kFBShadowTypeZLightMapProjectiveTexture = property(
        doc="Similar to the Projective Light Map except that it uses a boolean algorithm to create a self-shadow.         ")
    pass
...
class FBAnimationLayerMergeOptions(Enumeration):
    ...
    """
    kFBAnimLayerMerge_SelectedLayers_SelectedProperties = property(
        doc="Merge the animation of the selected properties of the selected models from the selected layers to the selected layer with the lowest index.         ")
    kFBAnimLayerMerge_AllLayers_SelectedProperties = property(
        doc="Merge the animation of the selected properties of the selected models from all the layers to the BaseAnimation layer.         ")
    kFBAnimLayerMerge_SelectedLayers_AllProperties = property(
        doc="Merge the animation of all properties of the selected models from the selected layers to the selected layer with the lowest index.         ")
    kFBAnimLayerMerge_AllLayers_AllProperties = property(
        doc="Merge the animation of all properties of the selected models from all the layers to the BaseAnimation layer.         ")
    kFBAnimLayerMerge_SelectedLayers_CompleteScene = property(
        doc="Merge the animation of all properties from the selected layers to the selected layer with the lowest index.         ")
    kFBAnimLayerMerge_AllLayers_CompleteScene = property(
        doc="Merge the animation of all properties from all the layers to the BaseAnimation layer.         ")
    pass
...
class FBElementAction(Enumeration):
    ...
    kFBElementActionAppend = property(
        doc="Append the elements to the current scene elements (when loading or merging).         ")
    kFBElementActionMerge = property(
        doc="Merge the elements from the file in the current scene (when merging).         ")
...
class FBParallelScheduleType(Enumeration):
    ...
    kFBParallelScheduleSimple = property(
        doc="Simple parallel schedule, mainly analyze the task dependency based on Motion Hierarchy (scene graph), but don't across active constraint.         ")
    kFBParallelScheduleAdvanced = property(
        doc="Advanced parallel schedule, task dependency analyzation will be able to across ative constraint, and plus motion hierarchy.         ")
    pass
...
class FBEventDragAndDrop(FBEvent):
    ...
    Components = property(
        doc="<b>Read Property:</b> List of components drop. (it acces the same data as FBEventDragAndDrop.Get)         ")
...
class FBGlobalEvalCallbackTiming(Enumeration):
    ...
    """
    kFBGlobalEvalCallbackBeforeDAG = property(
        doc="Invoked before any DAG (Transformation & Deformation) evaluation tasks started in evaluation pipeline / thread.         ")
    kFBGlobalEvalCallbackAfterDAG = property(
        doc="Invoked after all DAG (Transformation & Deformation) evaluation tasks finished in evaluation pipeline / thread.         ")
    kFBGlobalEvalCallbackAfterDeform = property(
        doc="Invoked after all deformation tasks finsished in evaluation pipeline / thread.         ")
    kFBGlobalEvalCallbackSyn = property(
        doc="Invoked when both evluation & rendering pipelines / threads are stopped. Useful for some complicated scene change tasks to avoid race condition.         ")
    kFBGlobalEvalCallbackBeforeRender = property(
        doc="Invoked in rendering pipleline, before any rendering tasks start (immediately after clearing GL back buffer).         ")
    kFBGlobalEvalCallbackAfterRender = property(
        doc="Invoked in rendering pipleline, after any rendering tasks finsish (just before swapping GL back/front buffer).         ")
    pass
...
class FBTangentClampMode(Enumeration):
    ...
    kFBTangentClampModeClamped = property(
        doc="The tangent will be flattened when the key is placed at the same value as an adjacent key.         ")
    pass
...
class FBPickingMode(Enumeration):
    ...
    kFBPickingModeCount = property(
        doc="End of enum, this valued indicates the number of picking modes available.         ")
    pass
...
class FBFileFormatAndVersion(Enumeration):
    ...
    """
    kFBFBX2010 = property(
        doc="It's FBX Version 6. Note: it's not equivalent to MotionBuilder 2010 Native FBX format.         ")
...
class FBModelEvaluationTaskType(Enumeration):
    ...
    kFBModelEvaluationBBox = property(
        doc="Model's bouding box computation task (approximately for deformable model)         ")
...
class FBDisplayMode(Enumeration):
    ...
    kFBDisplayModeCount = property(
        doc="End of enum, this value indicates the number of display modes available.         ")
    pass
...
class FBKeyingGroupType(Enumeration):
    ...
    """
    kFBKeyingGroupGlobal = property(
        doc="All selected objects with the same properties as those defined in the keying group will be keyed.         ")
    kFBKeyingGroupObjectType = property(
        doc="All selected objects of the specified type in the keying group with the same properties as those defined in the keying group will be keyed.         ")
    kFBKeyingGroupLocal = property(
        doc="Only properties of objects specified in the keying group will be keyed.         ")
    pass
...
class FBEventConnectionNotify(FBEvent):
    ...
    NewPlug = property(
        doc="<b>Read Only Property:</b> New plug created by the action. (Mostly used by merge/replace)         ")
...
class FBDisplayWhat(Enumeration):
    ...
    kFBDisplay3dIcon = property(
        doc="3D icons are displayed (3D icons are 3D elements that do not exist in the scene).         ")
...
class FBTriggerStyle(Enumeration):
    ...
    """
    kFBTriggerStyleContinue = property(
        doc="Previously triggered clips that are still playing won't be stopped and mixing will occur.         ")
    kFBTriggerStyleCut = property(doc="Previously triggered clips that are still playing will be stopped.         ")
    kFBTriggerStyleToggle = property(
        doc="If a previously triggered clip is playing, it will only be stopped, otherwise a new starts playing. No mixing and no loop.         ")
    pass
...
class FBAccessMode(Enumeration):
    ...
    kFBAccessModeMemory = property(
        doc="Access data from memory, which means that it will copyed entirely into it.         ")
    pass
...
class FBShadowFrameType(Enumeration):
    ...
    """
    kFBShadowFrameTypeShadowReceiver = property(
        doc="Bases the shadow calculation on the shadow of the receiver.         ")
...
class FBStoryFolder(FBComponent):
    ...
    RecordClipPath = property(
        doc="<b>Read Write Property:</b> Path for story recording. Can be relative or full path.         ")
...
class FBAssetMng(FBComponent):
    ...
    MenuFlags = property(
        doc="<b>Read Write Property:</b> Flags specifing which menu items are added by the manager.         ")
...
class FBAnimationNode(FBComponent):
    ...
    FCurve = property(
        doc="<b>Read Write Property:</b> FCurve for animation. See sample: StartKeysAtCurrentTime.py.         ")
...
You can also delete a Menu item: this will remove the item from the menu as well as freeing its memory.To be notified when a menuitem is clicked, you can register using OnMenuActivate. This will send a FBEventMenu containing the name and the Id of the menu item that was clicked.See sample: FBMenu.py.
    ...
    OnMenuActivate = property(
        doc="<b>Event Property:</b> Register on this property to be notified when a menu item is clicked by the user.         ")
    pass
...
class FBRendererCallback(FBComponent):
    ...
    DefaultCameraBackPlateRendering = property(
        doc="<b>Read write Property:</b> Set true to use default camera back plate rendering; set false to disable it.         ")
    DefaultCameraFrontPlateRendering = property(
        doc="<b>Read write Property:</b> Set true to use default camera front plate rendering; set false to disable it.         ")
    DefaultLightGroundProjectionRendering = property(
        doc="<b>Read write Property:</b> Set true to use default light ground projection rendering; set false to disable it.         ")
    DefaultLightVolumeRendering = property(
        doc="<b>Read write Property:</b> Set true to use default light volume rendering; set false to disable it.         ")
    SupportIDBufferPicking = property(
        doc="<b>Read write Property:</b> Can this Renderer Callback support IDBuffer Picking.         ")
    pass
...
class FBPointCacheManager(FBComponent):
    ...
    AllowCacheResampling = property(
        doc="<b>Read Write Property:</b> Allow the resample models's existing point cache deformation when true.         ")
    AlwaysAskForPath = property(
        doc="<b>Read Write Property:</b> Always ask for the point cache file save path when true.         ")
    ApplyCacheOnNewModel = property(
        doc="<b>Read Write Property:</b> Duplicated the cached models, and assoicated the point cache to the new models.         ")
    ApplyGlobalTransform = property(
        doc="<b>Read Write Property:</b> Include no-deformable models and the global transform to Vertex Cache when true.         ")
    CacheAABBox = property(
        doc="<b>Read Write Property:</b> Cache AABBox (Axis Aligned Bounding Box) when true.         ")
    CacheNormal = property(doc="<b>Read Write Property:</b> Cache normal when true.         ")
    CreateFilePerFrameCache = property(
        doc="<b>Read Write Property:</b> Create the point cache file for each frame when true.         ")
    CreateMultiChannelCache = property(
        doc="<b>Read Write Property:</b> Create a single multiple channel point cache file for all models when true.         ")
    ...
    NewModelRoot = property(
        doc="<b>Read Write Property:</b> Valid only when ApplyCacheOnNewModel is on. Create New Models under NewModelRoot. otherwise, a NULL model will be created.         ")
    SaveEveryFrame = property(doc="<b>Read Write Property:</b> Recording Frequency.         ")
    SetTransformReference = property(
        doc="<b> Action Property:</b> Set the model's current transformation as the reference.         ")
    pass
...
class FBStoryClip(FBComponent):
    ...
    FrameRate = property(
        doc="<b>Read Write Property:</b> Frame rate value. Only effective when UseSystemFrameRate is false.         ")
    ...
    Loaded = property(
        doc="<b>Read Write Property:</b> If true, clip file is loaded into memory and can be evaluated (will affect track content).         ")
    ...
    Rotation = property(
        doc="<b>Read Write Property:</b> Animation clip's rotation offset. Refer to class notes to learn more about how this is applied.         ")
    Scale = property(
        doc="<b>Read Write Property:</b> Animation clip's scaling (some don't support this property)         ")
    ShotActionStart = property(
        doc="<b>Read Write Property:</b> If not in locked shot mode (time discontinuity enabled), this time can be different from the Clip->Start property.         ")
    ...
    ShowGhostClipMode = property(
        doc="<b>Read Write Property:</b> Show the ghost depending on the time. See FBStoryClipShowGhostMode         ")
    SolvingMode = property(
        doc="<b>Read Write Property:</b> Solve Modes for story character clips. See FBStoryClipSolveMode         ")
    ...
    Translation = property(
        doc="<b>Read Write Property:</b> Animation clip's translation offset. Refer to class notes to learn more about how this is applied.         ")
...
class FBPropertyEnum(FBProperty):
    ...
    """
    Data = property(
        doc="Return the string associated with the index. Will return None when no value is associated.         ")
    pass
...
class FBFbxOptions(FBComponent):
    ...
    CacheSize = property(
        doc="<b>Read Write Property:</b> The Cached buffer size used to accelerate IO system.         ")
    ...
    CharacterFacesAnimation = property(
        doc="<b>Read Write Property:</b> Handling of the Character Faces animation.         ")
    ...
    ClearSelectionBeforeSave = property(
        doc="<b>Read Write Property:</b> Set to true if the current selected objects shouldn't saved when call FBApplication::SaveCharacterRigAndAnimation.         ")
    ConsiderMuteSolo = property(
        doc="<b>Read Write property:</b> Consider the mute/solo settings to identify identical layer when merging.         ")
    ...
    CopyCharacterExtensions = property(
        doc="<b>Read Write Property:</b> pCopyMissingExtensions Set to true if the character extensions on the rig in the file should be copied to the target rig.         ")
    ...
    EmbedMedia = property(
        doc="<b>Read Write Property:</b> Embed all media in the FBX file itself. When saving in ASCII mode it is not possible to embed media.         ")
    FileFormatAndVersion = property(
        doc="<b>Read Write Property:</b> File format and version chosen to save the scene.         ")
    FileReference = property(doc="<b>Read Write property:</b> Load/Save scene as FileReference.         ")
    FileReferenceEdit = property(
        doc="<b>Read Write Property:</b> Load/Save the edits made to referenced objects or not.         ")
    ...
    IgnoreConflicts = property(
        doc="<b>Read Write Property:</b> Set to true to ignore conflicts between objects in character extensions and objects in the scene. Conflicting objects will be merged in the extension         ")
    KeepTransformHierarchy = property(
        doc="<b>Read Write Property:</b> Indicate whether we keep transform hierarchy when SaveSelectedModelsOnly is true. Default value is false to ensure consistent behavior with SaveSelected operation via file menu.         ")
    ...
    NamespaceList = property(
        doc="<b>Read Write Property:</b> A list of namespaces separated by '~', On Load, duplicate the loaded objects into each namespace in the list.         ")
    ...
    PhysicalProperties = property(
        doc="<b>Read Write Property:</b> Handling of the Physical Properties elements.         ")
    PhysicalPropertiesAnimation = property(
        doc="<b>Read Write Property:</b> Handling of the Physical Properties animation.         ")
    Poses = property(doc="<b>Read Write Property:</b> Handling of the Poses elements.         ")
    ProcessAnimationOnExtension = property(
        doc="<b>Read Write Property:</b> Set to true if animation on character extensions should also be transferred.         ")
    RemoveConstraintReference = property(
        doc="<b>Read Write Property:</b> Set to true if we should remove constraint reference.         ")
    RemoveEmptyLayer = property(
        doc="<b>Read Write property:</b> Remove empty animation layers that are in additive mode, without child or parent.         ")
    ReplaceControlSet = property(
        doc="<b>Read Write Property:</b> Set to true if the character extensions (and their children) should be saved when call FBApplication::SaveCharacterRigAndAnimation.         ")
    ResetDOF = property(
        doc="<b>Read Write Property:</b> Set to true if we should change the limits on the target rig.         ")
    ResetHierarchy = property(
        doc="<b>Read Write Property:</b> Set to true if we should reset the character hierarchy.         ")
    RetargetOnBaseLayer = property(
        doc="<b>Read Write Property:</b> If the transfer method is retarget, set this parameter to control where the retarget correction will be made (on base layer or on another layer).         ")
    SaveCharacter = property(
        doc="<b>Read Write Property:</b> Set to true if the character should be saved when call FBApplication::SaveCharacterRigAndAnimation.         ")
    SaveCharacterExtensions = property(
        doc="<b>Read Write Property:</b> Set to true if the character extensions (and their children) should be saved when call FBApplication::SaveCharacterRigAndAnimation.         ")
    SaveControlSet = property(
        doc="<b>Read Write Property:</b> Set to true if the rig (and its children) should be saved when call FBApplication::SaveCharacterRigAndAnimation.         ")
    SaveSelectedModelsOnly = property(
        doc="<b>Read Write Property:</b> Indicate that only the selected models will be saved.         ")
    Scripts = property(doc="<b>Read Write Property:</b> Handling of the Scripts elements.         ")
    SetPropertyStaticIfPossible = property(
        doc="<b>Read Write Property:</b> Set to false if want to keep properties' animated flag even when they are not really animated(no keyframe data) while retrieving/storing. See sample: SetPropertyStaticIfPossibleOption.py.         ")
    ...
    ShowFileDialog = property(
        doc="<b>Read Write Property:</b> Set to true if want to pop up dialog for FileName, Format, Embed, Compression, UseTakeName, OneTakePerFile.         ")
    ShowOptionsDialog = property(
        doc="<b>Read Write Property:</b> Set to true if want to pop up options dialog for detail settings.         ")
    ...
    StoryAnimation = property(
        doc="<b>Read Write Property:</b> Handling of the Story animation (animatable properties on story objects).         ")
    TakeSpan = property(
        doc="<b>Read Write Property:</b> Indicate how the take start and end point should be set. By default it is read from the file.         ")
    ...
    TransferMethod = property(
        doc="<b>Read Write Property:</b> How should the animation should be transfered on the target rig.         ")
    ...
    UseASCIIFormat = property(
        doc="<b>Read Write Property:</b> Indicate if the resulting FBX file will be in binary or ASCII mode.         ")
...
class FBProfiler(FBComponent):
    ...
    ActiveSampling = property(
        doc="<b>Read/Write Property:</b> Activate the sampling for time events. Call before quering for FBProfileTimeEvent.         ")
    BufferSize = property(
        doc="<b>Read/Write Property:</b> Buffer size for average and timing computation (maximum value 200).         ")
    EvaluationDepth = property(
        doc="<b>Read/Write Property:</b> Specify the depth of evaluation profiling for data collection (maximum value is 10).         ")
    FrameReference = property(
        doc="<b>Read/Write Property:</b> Draw task cycles in relation to main thread cycle time - frame cycle (percentage display).         ")
    ProfilingMode = property(
        doc="<b>Read/Write Property:</b> Profiling collection modes, including disabling all profiling.         ")
    pass
...
Keys can also be set and used with Key(), GotoNextKey(), and GotoPreviousKey(). All actions are performed by default on the current take. The is the MotionBuilder default take, unless you have multiple takes in your scene. To switch between takes, use FBTake.See samples: ShotTrackSetupTool.py, RenderLayers.py, CameraSwitcher.py, BloopSlate.py, RecordLight.py, Timeline.py, CreateProfilingEventsLog.py, MirrorPoseOverTime.py, MultiLayerKeying.py, StartDevice.py, StopDevice.py, TimeCodeKeying.py.
    ...
    RecordingSamplingPeriod = property(
        doc="<b>Read Write Property:</b> Sampling period for the model recording.         ")
    SnapMode = property(doc="<b>Read Write Property:</b> Set the transport control snap mode.         ")
    TransportTimeFormat = property(
        doc="<b>Read Write Property:</b> Current Time Mode of the transport controls.         ")
    ZoomWindowStart = property(
        doc="<b>Read Write Property:</b> Starting time of the transport control zoom window.         ")
    ZoomWindowStop = property(
        doc="<b>Read Write Property:</b> Stopping time of the transport control zoom window.         ")
    pass
...
class FBImage(FBComponent):
    ...
    InterleaveType = property(
        doc="<b>Read Only Property:</b> Image interleave type. Only meaningful if image type is field.         ")
...
class FBPropertyAnimatable(FBProperty):
    ...
    Data = property(
        doc="<b>Read Write Property:</b> The property data value. Type of this depends on the subclass of FBPropertyAnimatable (ex: in a FBPropertyAnimatableInt, Data is of type int).         ")
    pass
...
class FBRenderer(FBComponent):
    ...
    AdvancedLightingMode = property(
        doc="<b>Read write Property:</b> Turn on/off advanced lighting setting UI widgets.         ")
    AdvancedMaterialMode = property(
        doc="<b>Read write Property:</b> Turn on/off advanced material setting UI widgets.         ")
    AutoEvaluate = property(
        doc="<b>Read Write Property:</b> Indicate if a call to RenderBegin will also cause a re-evaluation of the scene.         ")
    Background = property(doc="<b>Read Write Property:</b> The renderer.         ")
    CurrentCamera = property(
        doc="<b>Read Write Property:</b> Current camera. if UseCameraSwitcher is on, this will Get/Set camera switcher's current camera;         ")
    CurrentPaneCallbackIndex = property(
        doc="<b>Read Write Property:</b> Current Pane's Renderer Callback Index.         ")
    CurrentPaneCallbackPrefIndex = property(
        doc="<b>Read Write Property:</b> Current Pane's Renderer Callback Preference Index.         ")
    DisplayNormals = property(doc="<b>Read Write Property:</b> Display model normals in main viewer.         ")
    DisplaySetUpdateId = property(
        doc="<b>Read Only Property:</b> Current DisplaySet Update Id. Add/Delete models, Show/Hide models will affect DisplaySet.         ")
    ...
    FrustumCulling = property(
        doc="<b>Read Write Property:</b> Turn on/off the early frustum culling optimization.         ")
    IDBufferDisplay = property(
        doc="<b>Read write Property:</b> Render Model's unique Color ID into color Buffer (used for picking)         ")
    IDBufferPicking = property(
        doc="<b>Read write Property:</b> Use ID (Color) Buffer for picking, instead of OpenGl selection buffer picking.         ")
    IDBufferPickingAlpha = property(
        doc="<b>Read write Property:</b> Those Semi-transparent (Alpha Blend) geometry(region) contribute less than this threshold, will be considered as invisible during ID picking.         ")
    ...
    RendererUpdateId = property(
        doc="<b>Read Only Property:</b> Current Render Update Id. DisplaySet update, material change, texture changes and shader change and other operations will trigger Renderer update.         ")
    Scene = property(doc="<b>Read Write Property:</b> Scene that the renderer will use/draw         ")
    SelectionForceSnapPointsDisplay = property(
        doc="<b>Read write Property:</b> Force show all feature points (pivots and etc) on selected models if true, ignore individual model's settings.         ")
    SelectionOverride = property(
        doc="<b>Read write Property:</b> Add transparent color override layer on selected models if true.         ")
    ...
    SelectionOverrideTransparency = property(
        doc="<b>Read write Property:</b> Selection override layer transparency.         ")
    ShowStats = property(
        doc="<b>Read Write Property:</b> Show the stats about FPS, Evaluation rate ... like when using Shift-F in main viewer.         ")
    UseCameraSwitcher = property(
        doc="<b>Read Write Property:</b> Activate/Deactivate usage of camera switcher for the first model view of main viewer.         ")
    pass
...
class FBApplication(FBComponent):
    ...
    CurrentActor = property(
        doc="<b>Read Write Property:</b> Indicate the current actor, as used by the character tool. Can be NULL. If not null, CurrentCharacter must be null, as the character tool works on only one item at a time.         ")
    CurrentCharacter = property(
        doc="<b>Read Write Property:</b> Indicate the current character, as used by the character tool. Can be NULL. If not null, CurrentActor must be null, as the character tool works on only one item at a time. See sample: CurrentCharacterGoToStancePose.py.         ")
...
class FBEvaluateManager(FBComponent):
    ...
    FrameSkipOptimization = property(
        doc="<b>Read/Write Property:</b> if true, apply frame skip optimization during playback. off-line rendering don't use frame skip optimization.         ")
    ...
    ParallelDeformation = property(
        doc="<b>Read/Write Property:</b> true if deformation is evaluated in parallel.         ")
    ParallelEvaluation = property(
        doc="<b>Read/Write Property:</b> true if parallel DAG schedule algorithm is being used. false when serial algorithm is being used.         ")
    ParallelPipeline = property(
        doc="<b>Read/Write Property:</b> true if transformation is evaluated in parallel.         ")
    ParallelScheduleType = property(
        doc="<b> DEPRICATED </b><b>Read/Write Property:</b> choose between serial and parallel DAG schedule algorithm. kFBParallelScheduleSimple and kFBParallelScheduleAdvanced will set ParallelEvalution to true. kFBParallelScheduleSerial will set ParallelEvalution to false         ")
...
class FBKeyControl(FBComponent):
    ...
    AutoKey = property(
        doc="<b>Read Write Property:</b> Enable/Disable Auto Key feature (key when moving 3D objects).         ")
    pass
...
The Python sample FBSystemEvents.py shows how to register a callback to FBSystem.See samples: FBSystemEvents.py, CameraSwitcher.py, BatchExportCharacterAnimationTool.py, ExportAnimationLibrary.py.
    ...
    ComputerName = property(
        doc="<b>Read Only Property:</b> Computer name. See sample: ShowMachineNameAndCameraNamePlusResolution.py.         ")
    ...
    FullScreenViewer = property(
        doc="<b>Read Write Property:</b> Indicates that the viewer is in full screen mode.         ")
    ...
    OnConnectionKeyingNotify = property(
        doc="<b>Event:</b> A keying event occurred when objects are being keyed.         ")
    OnConnectionNotify = property(
        doc="<b>Event:</b> A connection event occurred between objects in the system. See sample: FBSystemEvents.py.         ")
    OnConnectionStateNotify = property(
        doc="<b>Event:</b> A state change event occurred between objects in the system.         ")
    OnUIIdle = property(
        doc="<b>Event:</b> User-interface idle event. Useful callback for less frequent GUI refresh and etc. lightweight tasks (occur once per several frames).         ")
    OnVideoFrameRendering = property(
        doc="<b>Event:</b> A video frame rendering event occurred when the scene is being off-line rendered into video files.         ")
...
See also the C++ code sample in toolscene.See samples: InsertCurrentTake.py, DeleteUnusedMedia.py, MirrorPoseOverTime.py, SelectModelsWithNameContainingSubstring.py, SetAllCamerasBackgroundColorFromFirstSelectedCamera.py, StartDevice.py.
    ...
    def NamespaceGetContentList(self, pContentList, pNamespace, pModificationFlags, pRecursive, pTypeInfo,
                                pExactTypeMatch):
        """
        Get List of the namespace content.
...
class FBAnimationLayer(FBComponent):
    ...
    LayerMode = property(
        doc="<b>Read Write Property:</b> Layer mode. By default, the layer is in kFBLayerModeAdditive mode. Cannot be applied to the BaseAnimation Layer.         ")
    LayerRotationMode = property(
        doc="<b>Read Only Property:</b> Layer rotation mode. Cannot be applied to the BaseAnimation Layer.         ")
    Lock = property(
        doc="<b>Read Write Property:</b> If true, the layer is locked. You cannot modify keyframes on a locked layer.         ")
    Mute = property(
        doc="<b>Read Write Property:</b> If true, the layer is muted. A muted layer is not included in the result animation. Cannot be applied to the BaseAnimation Layer.         ")
    Solo = property(
        doc="<b>Read Write Property:</b> If true, the layer is soloed. When you solo a layer, you mute other layers that are at the same level in the hierarchy, as well as the children of those layers. Cannot be applied to the BaseAnimation Layer.         ")
    Weight = property(
        doc="<b>Read Write Property:</b> The weight value of a layer determines how much it is present in the result animation. Takes a value from 0 (the layer is not present) to 100. The weighting of a parent layer is factored into the weighting of its child layers, if any. BaseAnimation Layer always has a Weight of 100.         ")
    pass
...
class FBAudioClip(FBComponent):
    ...
    AccessMode = property(
        doc="<b>Read Write Property:</b> Specify the media data access mode between disk or memory.         ")
    ...
    ConstrainDstToTake = property(
        doc="<b>Read Write Property:</b> Indicates whether to constrain the EndPoint to the end of the take.         ")
    CurrentTake = property(doc="<b>Read Write Property:</b> The take this media belongs to.         ")
    Destination = property(
        doc="<b>Read Write Property:</b> The audio output destination where the clip will be played.         ")
    DstDuration = property(
        doc="<b>Read Write Property:</b> When not used in the Story, this specify when the clips stops playing.         ")
    DstEnd = property(doc="<b>Read Write Property:</b> Total duration of this audio clip.         ")
    DstIn = property(
        doc="<b>Read Write Property:</b> When not used in the Story, this specify when the clips begin to play.         ")
    ...
    Format = property(
        doc="<b>Read Only Property:</b> Data format of media, including rate, bits and channels count. You can typecast it to a FBAudioFmt.         ")
    InPoint = property(doc="<b>[Deprecated]Read Write Property:</b> Refer to DstIn.         ")
    LockClipSpeed = property(
        doc="<b>Read Write Property:</b> Indicates whether to lock the current playing speed.         ")
    ...
    Scrubbing = property(
        doc="<b>Read Write Property:</b> Control which clip (one at a time) can shuttle when playing a various speeds.         ")
    ...
    TakeSetsInPoint = property(
        doc="<b>[Deprecated]Read Write Property:</b> Indicates whether to lock the InPoint to the start of the take.         ")
    UseChannel = property(
        doc="<b>Read Write Property:</b> Enables you to control which track are used with stereo clips.         ")
    UseChannelMode = property(
        doc="<b>[Deprecated]Read Write Property:</b> Enables you to control which track are used with stereo clips.         ")
    pass
...
class FBVideoGrabber(FBComponent):
    ...
    def RenderSnapshot(self, pWidth, pHeight, pCameraLabel, pTimeCode, pSafeArea, pAxis, pGrid, pFrontPlate,
                       pBackPlate):
        """
...
class FBStory(FBComponent):
    ...
    """
    LockedShot = property(
        doc="<b>Read Write Property:</b> If true, shots will be locked (no time discontinuity).         ")
    ...
    NoneBlockingPostprocess = property(
        doc="<b>Read Write Property:</b> If true, record to disk will post process recorded data in low priority thread without affecting application performance. Clip in story will remain unloaded.         ")
    RecordToDisk = property(
        doc="<b>Read Write Property:</b> If true, record to story will record directly to disk.         ")
    ...
    SummaryClip = property(
        doc="<b>Read Write Property:</b> If true, summary clips for story folders will be created to help manipulating folder content.         ")
    pass
...
class FBLayout(FBVisualComponent):
    ...
    def AddRegion(self, pName, pTitle, pX, pXType, pXRelative, pMultX, pY, pYType, pYRelative, pMultY, pW, pWType,
                  pWRelative, pMultW, pH, pHType, pHRelative, pMultH):
        """
...
There is a few ways to get a handle on existing models in a scene: FBFindObjectsByName return a list of objects matching a pattern (can contain *). For usage, see: FindObjectsWithWildcard.py If you know the name of the model, use FBFindModelByLabelName, as demonstrated in FBComponent.py. FBGetSelectedModels can get a handle to an object which is derived from FBModel. It searches the scene for a model, based on the model's unique name and returns a list of all the selected things in the scene.See sample: ResetLocalTranslationRotation.py.
    ...
    BlendShapeDeformable = property(
        doc="<b>Read Write Property:</b> Model blend-shape deformable. Not Savable         ")
    ...
    GeometryUpdateId = property(
        doc="<b>Read Only Property:</b> model geometry (vertex data) related update id.         ")
    ...
    Pickable = property(
        doc="<b>Read Write Property:</b> Indicate if a model can be picked in the viewer. This has a default value of 'true'.         ")
    PointCacheDeformable = property(
        doc="<b>Read Write Property:</b> Model point cache deformable. Not Savable         ")
    PointCacheRecord = property(doc="<b>Read Write Property:</b> Record Point Cache for model? Not Savable         ")
    PostRotation = property(
        doc="<b>Read Write Property:</b> Post Rotation (considered if RotationActive is true)         ")
    PreRotation = property(
        doc="<b>Read Write Property:</b> Pre Rotation (considered if RotationActive is true)         ")
    PrimaryVisibility = property(
        doc="<b>Read Write Property:</b> Control the geometry render state. Geometry can still cast shadows even if this is turned off.         ")
    ...
    RotationMax = property(
        doc="<b>Read Write Property:</b> Max Rotation Limit (considered if RotationActive is true)         ")
    ...
    RotationMin = property(
        doc="<b>Read Write Property:</b> Min Rotation Limit (considered if RotationActive is true)         ")
    ...
    RotationSpaceForLimitOnly = property(
        doc="<b>Read Write Property:</b> Apply Post Rotation Matrix only for Limits?         ")
    ...
    Show = property(
        doc="<b>Read Write Property:</b> Indicate if the viewer should show the object, according to its visibility value. This has a default value of 'false'.         ")
    ...
    Textures = property(
        doc="<b>List:</b> Textures with Special UseType (Other than 'Color' which should connect to materials).         ")
    Transformable = property(
        doc="<b>Read Write Property:</b> Indicate if a model can be transformable in the viewer. This has a default value of 'true'.         ")
    Translation = property(doc="<b>Read Write Property:</b> Lcl translation.         ")
    UniqueColorId = property(
        doc="<b> Read Only Property:</b> Unique Color Id for color based viewer picking. Color channel values are in the range [0, 1] with 1.0/255 precision.         ")
    UpVector = property(doc="<b>Read Write Property:</b> UpVector model.         ")
    Visibility = property(
        doc="<b>Read Write Property:</b> Visibility of model. This can be overridden by the 'Show' property.         ")
    VisibilityInheritance = property(
        doc="<b>Read Write Property:</b> //!< When this value is set to True the Visibility of this model is also applied to all its descendants         ")
    pass
...
See sample: PropertyDrop.py.
    ...
    LargeInc = property(
        doc="<b>Read Write Property:</b> Indicate the large increment applied when click-draging on the property value (usually left-click-dragging)         ")
    Precision = property(
        doc="<b>Read Write Property:</b> Used to specify the width and precision of the value shown. A value of 7.2 indicates to show at minimum 7 numbers, with 2 decimals.         ")
    Property = property(doc="<b>Read Write Property:</b> Property to edit. Set to NULL to disable.         ")
    SliderMax = property(
        doc="<b>Read Write Property:</b> Should the property be editable using a slider, set the maximum value atainable with the slider.         ")
    SliderMin = property(
        doc="<b>Read Write Property:</b> Should the property be editable using a slider, set the minimum value atainable with the slider.         ")
    SmallInc = property(
        doc="<b>Read Write Property:</b> Indicate the small increment applied when click-draging on the property value (usually right-click-dragging)         ")
    pass
...
class FBSet(FBBox):
    ...
    Transformable = property(
        doc="<b>Read Write Property:</b> Controls if objects in the set are transformable.         ")
...
class FBPlotPopup(FBVisualComponent):
    ...
    EnablePlotCharacterExtension = property(
        doc="<b>Read Write Property:</b> Enable Plot Character Extension option for popup.         ")
    EnablePlotLockedProperties = property(
        doc="<b>Read Write Property:</b> Enable Plot Locked Properties option for popup.         ")
    EnablePlotTranslationOnRootOnly = property(
        doc="<b>Read Write Property:</b> Enable Plot Translation On Root Only option for popup.         ")
...
class FBHUDElement(FBBox):
    ...
    """
    Height = property(
        doc="<b>Read Write Property:</b> Specifies the height of HUD element on the screen. It's in pixel when ScaleByPercent is false and percentage when ScaleByPercent is true.         ")
    HorizontalDock = property(
        doc="<b>Read Write Property:</b> Specifies if the HUD element will be horizontally docked to the Left, Right, or Center.         ")
    Justification = property(
        doc="<b>Read Write Property:</b> Specifies if the justification of the HUD element is Left, Right, or Center.         ")
    PositionByPercent = property(
        doc="<b>Read Write Property:</b> When set to true, X and Y position values are in percentage, relative to the corresponding camera view dimension. Otherwise, they are absolute pixel values.         ")
    ScaleByPercent = property(
        doc="<b>Read Write Property:</b> When set to true, Scale is in percentage, relative to the corresponding camera view dimension. Otherwise, it is an absolute value.         ")
    ScaleUniformly = property(
        doc="<b>Read Write Property:</b> Specifies whether the width and height of HUD element will be scaled uniformly according to the initial aspect ratio.         ")
    Show = property(doc="<b>Read Write Property:</b> Specifies if the HUD element will be displayed or not.         ")
    VerticalDock = property(
        doc="<b>Read Write Property:</b> Specifies if the HUD element will be vertically docked to the Bottom, Top, or Center.         ")
    Visibility = property(doc="        ")
    Width = property(
        doc="<b>Read Write Property:</b> Specifies the width of HUD element on the screen. It's in pixel when ScaleByPercent is false and percentage when ScaleByPercent is true.         ")
    X = property(
        doc="<b>Read Write Property:</b> Specifies the horizontal position of the HUD element, relative to dock position and justification.         ")
    Y = property(
        doc="<b>Read Write Property:</b> Specifies the vertical position of the HUD element, relative to dock position and justification.         ")
    pass
...
class FBVideo(FBBox):
    ...
    KeepOnGPU = property(
        doc="<b>Read Write Property:</b> Don't auto flush from GPU if true. session runtime flag, won't be saved.         ")
    pass
...
class FBHUD(FBBox):
    ...
    Visibility = property(
        doc="<b>Read Write Property:</b> Indicate if the information will be displayed or not.         ")
    pass
...
class FBGroup(FBBox):
    ...
    Transformable = property(
        doc="<b>Read Write Property:</b> Controls if objects in the group are transformable.         ")
    pass
...
class FBHandle(FBBox):
    ...
    Image = property(
        doc="<b>List:</b> Image to be used in the handle display. Only the image at position 0 is used.         ")
    Manipulate = property(doc="<b>List:</b> Objects manipulated by the handle.         ")
    ManipulateRotation = property(
        doc="<b>List:</b> Objects manipulated by the handle. Only their rotation is affected.         ")
    ManipulateScaling = property(
        doc="<b>List:</b> Objects manipulated by the handle. Only their scaling is affected.         ")
    ManipulateTranslation = property(
        doc="<b>List:</b> Objects manipulated by the handle. Only their translation is affected.         ")
    pass
...
class FBSlider(FBVisualComponent):
    ...
    OnTransaction = property(
        doc="<b>Event:</b> Transaction begin/end (continuous value changes). This event property doesn't exist in pyfbsdk.         ")
...
class FBTabPanel(FBVisualComponent):
    ...
    TabStyle = property(
        doc="<b>Read Write Property:</b> Style of the tab panel, 0 creates normal tabs, 1 creates buttons to activate tabs.         ")
    pass
...
class FBCharacterExtension(FBKeyingGroup):
    ...
    IncludePartInFullBody = property(
        doc="<b>Read Write Property:</b> Include or not this extension when fullBody is active.         ")
    Label = property(doc="<b>Read Write Property:</b> The logical name of the extension, use for mirroring.         ")
    MirrorLabel = property(
        doc="<b>Read Write Property:</b> Enum that indicate which extension is used as mirror, 0 is none, 1 is self, 2-n represent the (ith - 2)character extension in the attached character excluding self.         ")
...
class FBPropertyConnectionEditor(FBVisualComponent):
    ...
    Property = property(
        doc="<b>Read Write Property:</b> Property to edit connections. Set to NULL to disable.         ")
    pass
...
class FBTree(FBVisualComponent):
    ...
    AllowCollapse = property(
        doc="<b>Read Write Property:</b> When OnCollapsing occurs, set this to true to allow collapse.         ")
    AllowExpansion = property(
        doc="<b>Read Write Property:</b> When OnExpanding occurs, set this to true to allow expansion.         ")
    AutoExpandOnDblClick = property(
        doc="<b>Read Write Property:</b> Allow automatic expand on double click, default is false.         ")
    AutoExpandOnDragOver = property(
        doc="<b>Read Write Property:</b> Allow automatic expand on drag over, default is false.         ")
    AutoScroll = property(
        doc="<b>Read Write Property:</b> If AutoScroll property is True then the tree window will be automatically scrolled when the user drags item(s) over the boundaries of the tree.         ")
    AutoScrollOnExpand = property(
        doc="<b>Read Write Property:</b> Allow automatic scroll on expand, default is true.         ")
    CheckBoxes = property(doc="<b>Read Write Property:</b> Draw check boxe for each node.         ")
    DeselectOnCollapse = property(
        doc="<b>Read Write Property:</b> Tells whether node are deselected if parent node is collapsed.         ")
    EditNodeOn2Select = property(
        doc="<b>Read Write Property:</b> Set to true, to allow automatic node editing on second select.         ")
    HighlightOnRightClick = property(doc="<b>Read Write Property:</b> Hightlight node on right click.         ")
    Indent = property(
        doc="<b>Read Write Property:</b> Use Indent to determine how far child nodes are indented from their parent nodes when the parent is expanded.         ")
    ...
    MultiSelect = property(
        doc="<b>Read Write Property:</b> Tells whether multiple selection is allowed or not.         ")
    NoSelectOnDrag = property(
        doc="<b>Read Write Property:</b> Tells whether node are selected if drag is start and node is not already selected.         ")
    NoSelectOnRightClick = property(
        doc="<b>Read Write Property:</b> Tells whether node are selected if right click on node.         ")
    ...
    OnCollapsing = property(
        doc="<b>Event:</b> Fired before the node collapse. To refuse collapsing, set AllowCollapse to false.         ")
    OnDblClick = property(
        doc="<b>Event:</b> Double-Click on a node of the tree. Use FBEventTreeSelect to cast event.         ")
    ...
    OnExpanding = property(
        doc="<b>Event:</b> Is fired before the node expand. To refuse expanding set AllowExpansion to false.         ")
...
To see how to create a custom property in Python, see CustomProperty.py.See samples: CustomProperty.py, SetAllToDoneInAllTakes.py.
    ...
    Percent = property(
        doc="<b>Read Write Property:</b> Percent completed for the operation. Must be used called in between ProgressBegin()/ProgressDone()         ")
    Text = property(
        doc="<b>Read Write Property:</b> Text to display on progress bar. Must be used in between ProgressBegin()/ProgressDone()         ")
    pass
...
@endcode
    ...
    LocalShadow = property(
        doc="<b>Read Write Property:</b> Creates an accurate projection of a shadow for each object.         ")
    ...
    ShadowIntensity = property(
        doc="<b>Read Write Property:</b> Controls the darkness of shadows cast by a selected object.         ")
    ShadowType = property(doc="<b>Read Write Property:</b> Indicate which shadow type is desired.         ")
    ShadowZOffset = property(
        doc="<b>Read Write Property:</b> Specifies the offset of the Live Shadow shader's plane from the original selected plane.         ")
...
@endcode
    ...
    FKFingerMultiplier = property(
        doc="<b>Read Write Property:</b> Used to augment the amount of FK propagation for unmarkered intermediate finger phalanges.         ")
    FKFingerTipMultiplier = property(
        doc="<b>Read Write Property:</b> Used to augment the amount of FK propagation for unmarkered finger tip phalanges.         ")
    FKThumbTipMultiplier = property(
        doc="<b>Read Write Property:</b> Used to augment the amount of FK propagation for unmarkered thumb tip phalanges.         ")
    ...
    HumanFingerLimits = property(
        doc="<b>Read Write Property:</b> Enables/Disables human finger limits during actor solve.         ")
    ...
    LeftHandIndexIndex = property(
        doc="<b>Read Write Property:</b> Used to set blending coefficients. Each of the 4 fingers can be a blend of the 4 finger. This is not available for thumbs.         ")
...
class FBVideoClip(FBVideo):
    ...
    PowerOfTwoHeight = property(
        doc="<b>Read Only Property:</b> Closest power of two value superior to height of image.         ")
    PowerOfTwoWidth = property(
        doc="<b>Read Only Property:</b> Closest power of two value superior to width of image.         ")
...
class FBModelPlaceHolder(FBBoxPlaceHolder):
    ...
    UseGlobalTransforms = property(
        doc="<b>Read Write Property:</b> Indicate if the translations are expressed in local or global mode.         ")
    pass
...
class FBCameraSwitcher(FBModel):
    ...
    CurrentCamera = property(
        doc="<b>Read Write Property:</b> Camera currently being used by the switcher. Set to NULL to turn on evalute switch, otherwise manual switch.         ")
    CurrentCameraIndex = property(
        doc="<b>Read Write Property:</b> Camera index currently being used by the switcher. Set to -1 to turn on evaluate switch.         ")
    pass
...
class FBStoryTrack(FBConstraint):
    ...
    ClipNameConvention = property(
        doc="<b>Read Write Property:</b> Naming convention for each new recording clip that is created. Can use special tags: <Name> <StartTCValue> <StartFrameValue> <StartDate> <StartTime> <TakeName>         ")
    ...
    ParentTrack = property(
        doc="<b>Read Only Property:</b> Parent track, if the track is of Character or Animation type.         ")
    PassThrough = property(
        doc="<b>Read Write Property:</b> Enable passthrough of animation if there is no clip on track animation is taken from other tracks of take         ")
    RecordClipPath = property(
        doc="<b>Read Write Property:</b> Path for story recording. Can be relative or full path.         ")
    RecordTrack = property(
        doc="<b>Read Write Property:</b> Path for story recording. Can be relative or full path.         ")
    ReferenceMode = property(
        doc="<b>Read Write Property:</b> Track composition mode, kFBStoryTrackOverride or kFBStoryTrackAdditive         ")
...
class FBVideoMemory(FBVideo):
    ...
    TextureOGLId = property(
        doc="<b>Read Write Property:</b> OpenGL texture buffer object id (GL_TEXTURE_2D type).         ")
    pass
...
@endcode
    ...
    Alpha = property(
        doc="<b>Read Write Property:</b> Controls the actual effect of the shader on the object. At 0.0 it does nothing, and at 1.0 it fully affects the object.         ")
    Contrast = property(
        doc="<b>Read Write Property:</b> Changes the contrast of the object when it reflects light.         ")
    Luminosity = property(
        doc="<b>Read Write Property:</b> Changes the brightness of the object when reflecting light.         ")
    Specular = property(
        doc="<b>Read Write Property:</b> Changes an object's level of shininess when it reflects light by affecting the specular highlight.         ")
    Transparency = property(
        doc="<b>Read Write Property:</b> Indicates the computation method of the transparency.         ")
...
class FBHUDTextElement(FBHUDElement):
    ...
    AdjustWidthToFitText = property(
        doc="<b>Read Write Property:</b> If On it it will adjust the width of a text element so that a text character's aspect ratio does not change as the content grows or shrinks.         ")
    ...
    ForceTimeCodeDisplay = property(
        doc="<b>Read Write Property:</b> Specifies if the display of time-related reference property will be in timecode format.         ")
    pass
...
class FBTool(FBLayout):
    ...
    StartSizeX = property(
        doc="<b>Read Property:</b> Starting Size. This is the initial size in X when the tool is opened. Default = 800         ")
    StartSizeY = property(
        doc="<b>Read Property:</b> Starting Size. This is the initial size in Y when the tool is opened. Default = 400         ")
    MaxSizeX = property(
        doc="<b>Read Property:</b> Maximum Size in X (Disabled in this version). A value of -1 means no maximum size.         ")
    MaxSizeY = property(
        doc="Maximum Size in Y (Disabled in this version). A value of -1 means no maximum size.         ")
    ...
    StartPosX = property(
        doc="<b>Read Property:</b> Starting Position in X. This is the initial position when the tool is opened. Default = 450         ")
    StartPosY = property(
        doc="<b>Read Property:</b> Starting Position in Y. This is the initial position when the tool is opened. Default = 450         ")
...
@endcode
    ...
    PathLengthInString = property(
        doc="<b>Read Only Property:</b> Path Length Display String According To The Current Unit.         ")
...
class FBVideoIn(FBVideo):
    ...
    FilePath = property(
        doc="<b>Read Write Property:</b> Location of the generated movie file after a recording session.         ")
    Online = property(
        doc="<b>Read Write Property:</b> If true, the device is online and will display the current video feed.         ")
    RecordAudio = property(
        doc="<b>Read Write Property:</b> If true, the device will also record audio during a recording session.         ")
    Recording = property(
        doc="<b>Read Write Property:</b> If true, the device will record during a recording session.         ")
    pass
...
class FBLayeredTexture(FBTexture):
    ...
    BackgroundColor = property(
        doc="<b>Read/Write Property:</b> Animatable Background color which is used to clear color buffer before composition.         ")
...
class FBCamera(FBModel):
    ...
    BackGroundPlaneDistance = property(
        doc="<b>Read Write Property:</b> Set the distance for the background plane.         ")
    BackGroundPlaneDistanceMode = property(
        doc="<b>Read Write Property:</b> Select mode for the background plane's distance.         ")
    ...
    Display2DMagnifierFrame = property(
        doc="<b>Read Write Property:</b> Enable/Disable the drawing of the 2D Magnifier frame box.         ")
    DisplayTurnTableIcon = property(
        doc="<b>Read Write Property:</b> Enable/Disable the drawing of the Turn Table icon.         ")
    FarPlaneDistance = property(doc="<b>Read Write Property:</b> Far plane distance.         ")
    FieldOfView = property(
        doc="<b>Read Write Property:</b> Field of View (used when in horizontal or vertical aperture modes).         ")
    FieldOfViewX = property(
        doc="<b>Read Write Property:</b> Field of View X angle (used in horizontal and vertical aperture mode).         ")
    FieldOfViewY = property(
        doc="<b>Read Write Property:</b> Field of View Y angle (used in horizontal and vertical aperture mode).         ")
    ...
    ForeGroundMaterialThreshold = property(
        doc="<b>Read Write Property:</b> Material threshold for a transparent foreground.         ")
    ForeGroundMedia = property(doc="        ")
    ForeGroundPlaneDistance = property(
        doc="<b>Read Write Property:</b> Set the distance for the foreground plane.         ")
    ForeGroundPlaneDistanceMode = property(
        doc="<b>Read Write Property:</b> Select mode for the foreground plane's distance.         ")
    ...
    SystemCamera = property(
        doc="<b>Read Only Property:</b> Indicate if this a producer (default or system) camera or a custom (user-created) camera.         ")
...
class FBVideoClipImage(FBVideoClip):
    ...
    MaxMipMapResolution = property(
        doc="<b>Read Write Property:</b> Maximum MipMap resolution will be loaded into GPU.         ")
...
class FBHUDBloopSlateElement(FBHUDFlashElement):
    ...
    BackgroundColor = property(
        doc="<b>Read Write Property:</b> Bloop slate background color, by default it is 100% transparent.         ")
    ...
    ShowAfterDelayOnRecordPlay = property(
        doc="<b>Read Write Property:</b> Delay before the bloop slate is displayed after recording has started.         ")
...
class FBHUDTimelineElement(FBHUDFlashElement):
    ...
    CutActiveColor = property(
        doc="<b>Read Write Property:</b> Specifies color of the Cut region when it is active.         ")
    CutIdleColor = property(
        doc="<b>Read Write Property:</b> Specifies color of the Cut region when it is idle.         ")
    HeadActiveColor = property(
        doc="<b>Read Write Property:</b> Specifies color of the Head region when it is active.         ")
    HeadDuration = property(doc="<b>Read Write Property:</b> Specifies duration of the Head region.         ")
    HeadIdleColor = property(
        doc="<b>Read Write Property:</b> Specifies color of the Head region when it is idle.         ")
    TailActiveColor = property(
        doc="<b>Read Write Property:</b> Specifies color of the Tail region when it is active.         ")
    TailDuration = property(doc="<b>Read Write Property:</b> Specifies duration of the Tail region.         ")
    TailIdleColor = property(
        doc="<b>Read Write Property:</b> Specifies color of the Tail region when it is idle.         ")
    pass
...
class FBCameraStereo(FBCamera):
    ...
    CenterCamera = property(
        doc="<b>Read Write Property: </b> This property hold the center camera connected to it. Must be either the master, left or right camera.         ")
    DisplayZeroParallaxPlane = property(doc="<b>Read Write Property: </b> Display the zero parallax plane.         ")
    FilmOffsetLeftCam = property(
        doc="<b>Read Write Property: </b> This property handles the film offset for the left camera. (inch)         ")
    FilmOffsetRightCam = property(
        doc="<b>Read Write Property: </b> This property handles the film offset for the right camera. (inch)         ")
    InteraxialSeparation = property(
        doc="<b>Read Write Property: </b> This property handles the distance between left and right cameras.         ")
    LeftCamera = property(
        doc="<b>Read Write Property: </b> This property hold the left camera connected to it.         ")
    ...
    RelativePrecompFileName = property(
        doc="<b>Read Write Property: </b> This property handles the relative precomp file name.         ")
    RightCamera = property(
        doc="<b>Read Write Property: </b> This property hold the right camera connected to it.         ")
    Stereo = property(
        doc="<b>Read Write Property: </b> //!< This property handles the types of Stereo camera.         ")
    ToeInAdjust = property(
        doc="<b>Read Write Property: </b> This property is to offset the computed toe-in effect when it's in Converged mode.         ")
    ZeroParallax = property(
        doc="<b>Read Write Property: </b> This property handles the distance on the camera view axis where the zero parallax plane occurs.         ")
    ...
    ZeroParallaxPlaneTransparency = property(
        doc="<b>Read Write Property: </b> Zero parallax plane transparency.         ")
    pass